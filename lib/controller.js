...
class Controller extends EventEmitter {
    ...
    async listConversations() {
        ...
        try {
            ...
            this.emit('error', new Error(`listConversations: ${err.message}`));
            return [];
        }
    }
     ...
     */
    async sendMessage(cascadeId, text, configOverride) {
        if (!this.ls) throw new Error('LS not connected');

        const cfg = configOverride ? { ...this.config, ...configOverride } : this.config;
        const body = buildSendBody(cascadeId, text, cfg);
