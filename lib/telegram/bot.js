...
async function startBot(controller) {
    ...
    bot.command('newfeature', async (ctx) => {
        const featureName = ctx.message.text.split(' ').slice(1).join(' ').trim();
        if (!featureName) {
            await ctx.reply(
                `${ce(EMOJI.CROSS)} 请提供功能名称\n例如: <code>/newfeature 增加登录页</code>`,
                { parse_mode: 'HTML' }
            );
            return;
        }
        ...
        try {
            const topic = await ctx.api.createForumTopic(ctx.chat.id, featureName, {
                icon_color: 0x6FB9F0,
            });
            await ctx.api.sendMessage(ctx.chat.id,
                `${ce(EMOJI.TOPIC)} 话题已创建: <b>${esc(featureName)}</b>\n请进入此话题发送指令！`,
                { parse_mode: 'HTML', message_thread_id: topic.message_thread_id }
            );
        } catch (err) {
            let hint = '';
            if (err.message.includes('not a forum')) {
                hint = '\n💡 此命令需要在开启了「话题」功能的群组中使用';
            }
            await ctx.reply(
                `${ce(EMOJI.CROSS)} 创建话题失败: ${esc(err.message)}${hint}`,
                { parse_mode: 'HTML' }
            );
        }
    });
