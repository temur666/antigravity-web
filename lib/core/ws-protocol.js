...
function buildSendBody(cascadeId, text, config = DEFAULT_CONFIG, extras = {}) {
    ...
    // 图片/媒体: 作为 imageData 项放入 items 数组（LS proto 要求）
    if (extras.media && extras.media.length > 0) {
        for (const m of extras.media) {
            if (m.data) {
                // base64 data 直接使用
                items.push({ imageData: { data: m.data, mimeType: m.mimeType } });
                ...
                try {
                    ...
                    items.push({ imageData: { data: buf.toString('base64'), mimeType: m.mimeType } });
                } catch (err) {
