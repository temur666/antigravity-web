 ...
 *   3. 合并多个 LS 的对话列表
 *   4. 管理每个 LS 的 AgentStreamClient
 *   5. 健康检查 + 动态增删实例
 *
 * 设计:
 *   - LSPool 继承 EventEmitter，提供统一的事件接口
 *   - 对外暴露 `.ls` (primary) 供 Controller 使用
 *   - 路由方法供 ConversationStore 使用
 */
...
class LSPool extends EventEmitter {
    ...
    onStreamUpdate(cb) { this._onStreamUpdateCb = cb; }
    onGetActiveSubscriptions(cb) { this._getActiveSubscriptionsCb = cb; }

    // 返回 primary 的 AgentStreamClient
    getStreamClient() {
        if (!this.primaryKey) return null;
    ...
    /**
     * 让 primary LS 从磁盘加载孤儿对话到内存索引
     * 调用 LoadTrajectory RPC，成功后更新路由表
     * @param {string} cascadeId
     * @returns {Promise<boolean>} 是否加载成功
     */
    async loadTrajectory(cascadeId) {
        const primary = this.instances.get(this.primaryKey);
        if (!primary) return false;
        try {
            await grpcCall(primary.port, primary.csrf, 'LoadTrajectory', { cascadeId });
            this.routeTable.set(cascadeId, this.primaryKey);
            console.log(`[LSPool] LoadTrajectory 成功: ${cascadeId.slice(0, 8)}... → Primary`);
            return true;
        } catch (err) {
            console.warn(`[LSPool] LoadTrajectory 失败: ${cascadeId.slice(0, 8)}... ${err.message}`);
            return false;
        }
    }
     ...
     * @param {number} port
     * @param {string} csrf
     * @returns {AgentStreamClient}
     */
    _createStream(key, port, csrf) {
        const stream = new AgentStreamClient(port, csrf);
 ...
 * @property {AgentStreamClient|null} stream
