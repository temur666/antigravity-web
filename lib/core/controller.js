...
const { normalizeSteps, normalizeStep } = require('./step-normalizer');
...
function createConversationState(cascadeId, source = 'external') {
    return {
        ...
        steps: [],
        totalSteps: 0,
        metadata: [],  // generatorMetadata，用于实时推送
        lastPollAt: null,
...
class Controller extends EventEmitter {
    ...
    async init() {
        ...
        // 注册工作区，确保 API 创建的对话不会触发 OUTSIDE_WORKSPACE 权限
    ...
    async cancelCascade(cascadeId) {
        if (!this.ls) throw new Error('LS not connected');
        await grpcCall(this.ls.port, this.ls.csrf, 'CancelCascadeInvocation', { cascadeId });
    }
    ...
    async getTrajectory(cascadeId) {
        ...
        let result = await grpcCall(this.ls.port, this.ls.csrf, 'GetCascadeTrajectory', { cascadeId });
        let data = result.data;

        // Fallback: LS 不认识此对话 → 调用 LoadTrajectory 从 .pb 文件加载后重试
        if (!data?.trajectory && data?.message === 'trajectory not found') {
            try {
                await grpcCall(this.ls.port, this.ls.csrf, 'LoadTrajectory', { cascadeId });
                result = await grpcCall(this.ls.port, this.ls.csrf, 'GetCascadeTrajectory', { cascadeId });
                data = result.data;
            } catch (err) {
                console.warn(`[Controller] LoadTrajectory fallback failed for ${cascadeId.slice(0, 8)}: ${err.message}`);
            }
        }
        ...
        // 返回时用 normalized steps 替换原始 steps
        // 避免下游（server.js）拿到 LS 原始字段名（如 absolutePathUri）
        if (data?.trajectory) {
            const conv = this.conversations.get(cascadeId);
            return {
                ...data,
     ...
     * 向 LS 注册工作区目录
     * 
     * 背景: LS 通过 LSP initialize 请求获取工作区（IDE 场景）。
     * 当通过 API 创建对话时没有 LSP 通道，LS 不知道工作区范围，
     * 导致文件操作被标记为 OUTSIDE_WORKSPACE 需要用户审批。
     * 
     * 解决: 主动调用 AddTrackedWorkspace + SetWorkingDirectories
     * 让 LS 知道哪些目录是可信的工作区。
     * 
     * @private
     */
    async _registerWorkspace() {
        ...
        try {
            // AddTrackedWorkspace: 注意参数是纯路径，不是 URI
            await grpcCall(this.ls.port, this.ls.csrf, 'AddTrackedWorkspace', {
                workspace: homeDir,
            });

            // SetWorkingDirectories: 注意参数是 URI 格式
            await grpcCall(this.ls.port, this.ls.csrf, 'SetWorkingDirectories', {
                directoryUris: [workspaceUri],
            });

            console.log(`[Controller] Workspace registered: ${homeDir}`);
        } catch (err) {
            // 非致命错误，IDE 的 LS 可能已有工作区注册
            console.warn(`[Controller] Workspace registration failed (non-fatal): ${err.message}`);
        }
    }
