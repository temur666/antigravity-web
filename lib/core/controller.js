...
class Controller extends EventEmitter {
    ...
    _onStreamChange(cascadeId) {
        ...
        if (elapsed >= throttleMs) {
            // 距上次 fetch 已够久，立即执行
            console.log(`[ctrl] _onStreamChange ${cascadeId.slice(0, 8)}: immediate fetch (elapsed=${elapsed}ms)`);
            doFetch();
        } else {
            // 设置尾调用，保证窗口结束后还会执行一次
            const remaining = throttleMs - elapsed;
            console.log(`[ctrl] _onStreamChange ${cascadeId.slice(0, 8)}: trailing in ${remaining}ms`);
    ...
    async _fetchAndDiff(cascadeId) {
        ...
        console.log(`[ctrl] _fetchAndDiff ${cascadeId.slice(0, 8)}: oldSteps=${conv.steps.length} newSteps=${newSteps.length} added=${diff.added.length} updated=${diff.updated.length} status=${newStatus} subs=${conv.subscribers.size}`);
