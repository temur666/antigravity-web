...
testAsync('newChat success', async () => {
    const grpc = require('../../ls/grpc');
    const oldCall = grpc.grpcCall;
    const store = new ConversationStore(createMockLSManager(true));
    try {
        grpc.grpcCall = async () => ({ data: { cascadeId: 'mock-id' } });
...
testAsync('sendMessage success', async () => {
    const grpc = require('../../ls/grpc');
    const oldCall = grpc.grpcCall;
    const store = new ConversationStore(createMockLSManager(true));
    try {
        let callCount = 0;
        grpc.grpcCall = async () => { callCount++; return { status: 200, data: {} }; };
        await store.sendMessage('mock-id', 'hello');
        assert.strictEqual(callCount, 1);
        const conv = store.conversations.get('mock-id');
...
testAsync('getTrajectory success', async () => {
    const grpc = require('../../ls/grpc');
    const oldCall = grpc.grpcCall;
    const store = new ConversationStore(createMockLSManager(true));
    try {
        ...
        const traj = await store.getTrajectory('mock-id');
        assert(traj.trajectory.steps.length === 1);
        const conv = store.conversations.get('mock-id');
