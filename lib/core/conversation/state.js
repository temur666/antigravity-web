...
class ConversationStore {
    ...
    async sendMessage(cascadeId, text, configOverride, extras) {
        ...
        console.log(`[Trace:5-gRPC] BEFORE SendUserCascadeMessage | traceId=${traceId} cascadeId=${cascadeId.slice(0, 8)}`);
        let result;
        if (this._isPool) {
        ...
        console.log(`[Trace:5-gRPC] AFTER SendUserCascadeMessage | traceId=${traceId} status=${result.status}`);
