...
function normalizeKnowledgeArtifacts(raw) {
    const content = raw.content || '';
    const items = [];

    // 提取 <knowledge_item> 块
    const blockRegex = /<knowledge_item>([\s\S]*?)<\/knowledge_item>/g;
    let match;
    while ((match = blockRegex.exec(content)) !== null) {
        ...
        let basePath = '';
        const artifactPaths = [];
        let section = 'header'; // header | summary | paths
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith('# ')) {
                ...
                lastAccessed = trimmed.replace('Last accessed:', '').trim();
            } else if (trimmed.startsWith('Summary:')) {
                summary = trimmed.replace('Summary:', '').trim();
                section = 'summary';
            } else if (trimmed.startsWith('Paths to artifacts')) {
                // "Paths to artifacts (relative to /path/to/base):"
                const baseMatch = trimmed.match(/\(relative to ([^)]+)\)/);
                if (baseMatch) basePath = baseMatch[1];
                section = 'paths';
            } else if (section === 'summary' && trimmed && !trimmed.startsWith('Paths to')) {
                summary += ' ' + trimmed;
            } else if (section === 'paths' && trimmed) {
                artifactPaths.push(trimmed);
            }
        }

        if (title) {
            items.push({ title, lastAccessed, summary, basePath, artifactPaths });
        }
    }
...
function normalizeCodeAcknowledgement(raw) {
    return {
...
const PAYLOAD_KEYS = {
    ...
    'CORTEX_STEP_TYPE_KNOWLEDGE_ARTIFACTS': 'knowledgeArtifacts',
};
...
const NORMALIZERS = {
    ...
    'CORTEX_STEP_TYPE_CODE_ACKNOWLEDGEMENT': normalizeCodeAcknowledgement,
    'CORTEX_STEP_TYPE_KNOWLEDGE_ARTIFACTS': normalizeKnowledgeArtifacts,
};
