 ...
 *   - Diff 引擎 (steps 增量计算)
 *   - 流式通知处理 (StreamAgentStateUpdates 结构化数据)
 *   - Polling fallback
 *   - 广播 (seq 分配 + 环形缓冲区)
 *
 * v2 变更 (AgentStreamClient):
 *   - status 变化: 直接广播, 不拉取
 *   - metadata 更新: 直接广播, 不拉取
 *   - step 新增/更新 (非 PLANNER_RESPONSE): 直接广播, 不拉取
 *   - PLANNER_RESPONSE 变化: 必须拉取文本 (stream 不推 response 字段)
 */
...
class ConversationSync extends EventEmitter {
    /**
     * @param {import('../ls/pool').LSPool} lsManager
    ...
    constructor(lsManager, store) {
        ...
        /** @type {Set<string>} 竞态锁: 正在 fetch 的 cascadeId */
        this._fetchLocks = new Set();
        /** @type {Set<string>} 锁期间有新请求待执行 */
        this._fetchPending = new Set();

        // 注册 stream 回调到 LSPool
        lsManager.onStreamChange((cascadeId) => this._onStreamChange(cascadeId));
        lsManager.onStreamUpdate((payload) => this._onStreamUpdate(payload));
    ...
    subscribe(cascadeId, ws, lastSeq = null) {
        ...
        if (lastSeq != null && conv.eventBuffer.length > 0) {
            ...
            if (missed.length > 0) {
                try {
                    const isOpen = typeof ws.isOpen === 'function' ? ws.isOpen() : ws.readyState === 1;
                    if (isOpen) {
        ...
        // 启动流式订阅 — Pool 模式路由到正确 LS 的 stream
        const streamClient = typeof this._lsManager.getStreamForConversation === 'function'
            ? this._lsManager.getStreamForConversation(cascadeId)
            : this._lsManager.getStreamClient();
    ...
    unsubscribe(cascadeId, ws) {
        ...
        if (conv) {
            ...
            if (conv.subscribers.size === 0) {
                ...
                // Clear debouncer and last fetch cache
                const timer = this._changeDebouncers.get(cascadeId);
                if (timer) {
                    clearTimeout(timer);
                    this._changeDebouncers.delete(cascadeId);
                }
                this._lastFetchTimes.delete(cascadeId);
            }
        }
    }
    ...
    /**
     * 处理 AgentStreamClient 的 'update' 事件
     * 直接消费结构化数据，仅 needsTextFetch 时拉取全量
     * @private
     * @param {{ cascadeId: string, data: object }} payload
     */
    _onStreamUpdate({ cascadeId, data }) {
        const conv = this._store.getOrCreate(cascadeId);
        ...
        if (data.status) {
            ...
            if (newStatus !== oldStatus) {
                ...
                // 对话完成检测：RUNNING → IDLE 且 stepCount > 3 → 触发归档
        ...
        if (data.metadataUpdate) {
            // 合并 metadata (按 index 更新)
            const mu = data.metadataUpdate;
            if (mu.metadatas && mu.metadatas.length > 0) {
                // 简化处理: 直接追加/替换
                const newMeta = [...conv.metadata];
                for (let i = 0; i < mu.metadatas.length; i++) {
                    const idx = mu.indices?.[i] ?? newMeta.length;
                    newMeta[idx] = mu.metadatas[i];
                }
        ...
        // ── Steps 更新 ──
        if (data.stepsUpdate) {
            const su = data.stepsUpdate;
            // 更新 totalLength
            if (su.totalLength) {
                conv.totalSteps = su.totalLength;
            }

            if (data.needsTextFetch) {
                // 有 PLANNER_RESPONSE → 必须拉文本 (stream 不推 response 字段)
                // 用 throttle 避免过于频繁
                this._onStreamChange(cascadeId);
            } else {
                // 非文本 step (USER_INPUT, CHECKPOINT, etc.) → 直接广播
                // 这些 step 的结构完整，不需要二次拉取
                for (let i = 0; i < su.steps.length; i++) {
                    const stepIdx = su.indices?.[i] ?? i;
                    const step = su.steps[i];
                    const normalizedStep = this._normalizeStreamStep(step);

                    if (stepIdx >= conv.steps.length) {
                        // 新增 step
                        conv.steps[stepIdx] = normalizedStep;
                        this._broadcastWithSeq(conv, {
    ...
    /**
     * 将 stream 推来的 step 转为与 normalizeSteps 兼容的格式
     * @private
     * @param {object} step - 原始 AgentStateUpdate 中的 step
     * @returns {object}
     */
    _normalizeStreamStep(step) {
        // step 已经是 JSON 格式, 做最小化的字段映射
        // type/status 保持原值 (CORTEX_STEP_TYPE_* / CORTEX_STEP_STATUS_*)
        return {
            type: step.type || '',
            status: step.status || '',
            metadata: step.metadata || {},
            userInput: step.userInput || null,
            plannerResponse: step.plannerResponse || null,
            error: step.error || null,
            checkpoint: step.checkpoint || null,
            // 其他可能的字段
            codeAction: step.codeAction || null,
     ...
     * (仅在 needsTextFetch=true 时被调用，或作为 fallback)
     * @private
     */
     ...
     * 加了 per-cascadeId 竞态锁：同一对话的 fetch+diff 不会并发执行。
     * 如果锁期间有新请求，完成后自动重跑一次，确保不丢数据。
     * @private
     */
    async _fetchAndDiff(cascadeId) {
        // 竞态锁：已有同一 cascadeId 的 fetch 在跑，标记 pending 后返回
        if (this._fetchLocks.has(cascadeId)) {
            this._fetchPending.add(cascadeId);
            return;
        }

        this._fetchLocks.add(cascadeId);
        try {
            await this._doFetchAndDiff(cascadeId);
        } finally {
            this._fetchLocks.delete(cascadeId);
            // 锁期间又有新请求 → 重跑一次
            if (this._fetchPending.has(cascadeId)) {
                this._fetchPending.delete(cascadeId);
                // 异步重跑，避免栈溢出
                this._fetchAndDiff(cascadeId).catch(err => {
                    this.emit('error', new Error(`fetch-pending ${cascadeId}: ${err.message}`));
                });
            }
        }
    }
    ...
    /**
     * 实际的 fetch + diff 逻辑（由 _fetchAndDiff 保护）
     * @private
     */
    async _doFetchAndDiff(cascadeId) {
        const conv = this._store.conversations.get(cascadeId);
        if (!conv) return;

        let result;
        if (typeof this._lsManager.grpcCallRouted === 'function') {
        ...
        if (newStatus !== oldStatus) {
            ...
            // 对话完成检测：RUNNING → IDLE 且 stepCount > 3 → 触发归档
            if (oldStatus === 'RUNNING' && newStatus === 'IDLE' && newSteps.length > 3) {
                this.emit('conversation_completed', { cascadeId, stepCount: newSteps.length });
            }
        }
     ...
     */
    async pollOnce() {
        if (!this._lsManager.isHealthy()) return;
