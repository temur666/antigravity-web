...
class StreamClient extends EventEmitter {
     ...
     */
    subscribe(cascadeId) {
        if (this._streams.has(cascadeId)) {
            console.log(`[stream] subscribe ${cascadeId.slice(0, 8)}: already subscribed, skip`);
            return;
        }
        console.log(`[stream] subscribe ${cascadeId.slice(0, 8)}: starting...`);
    ...
    _doSubscribe(cascadeId, protocol) {
        ...
        const req = mod.request({
            ...
            console.log(`[stream] ${cascadeId.slice(0, 8)}: HTTP ${res.statusCode}`);
            ...
            res.on('end', () => {
                console.log(`[stream] ${cascadeId.slice(0, 8)}: stream ended (res.on end)`);
                this._cleanup(cascadeId);
        ...
        req.on('error', (err) => {
            console.log(`[stream] ${cascadeId.slice(0, 8)}: req error: ${err.message} (code=${err.code})`);
        ...
        while (buf.length >= 5) {
            ...
            try {
                ...
                if (newVersion && newVersion !== oldVersion) {
                    ...
                    if (oldVersion) {
                        console.log(`[stream] ${cascadeId.slice(0, 8)}: change v=${newVersion}`);
                        this.emit('change', { cascadeId, version: newVersion });
                    } else {
                        console.log(`[stream] ${cascadeId.slice(0, 8)}: snapshot v=${newVersion}`);
